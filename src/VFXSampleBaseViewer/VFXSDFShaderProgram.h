...
namespace Leviathan
{
	...
	const char* vfx_sdf_frag =
		...
		"vec3 RayMarching(vec3 frag_world_coord, vec3 frag_obverse_dir, int step_count)"
		"{"
		...
		"		vec3 step_vector = frag_obverse_dir * (weight + 0.1);"
		...
		"	vec3 tex_coord = RayMarching(frag_world_vector, obverse_dir, 30);"
