...
namespace Leviathan
{
	namespace Renderer
	{
		OpenGLMaterial::OpenGLMaterial(Color ambient /*= { 0.0f, 0.0f, 0.0f }*/, Color diffuse /*= { 0.0f, 0.0f, 0.0f }*/, Color specular /*= { 0.0f, 0.0f, 0.0f }*/, float fShininess /*= 32.0f*/) :
			m_CAmbient(ambient),
			m_CDiffuse(diffuse),
		...
		bool OpenGLMaterial::ApplyMaterial(GLuint program)
		{
			GLint nErrorCode = -1;
		...
		bool OpenGLMaterial::UnApplyMaterial(GLuint program)
		{
			for (unsigned i = 0; i < m_pTexture2DVec.size(); i++)
			{
