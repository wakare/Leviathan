...
void LevSceneObjectTreeView::dragEnterEvent(QDragEnterEvent * event)
{
...
void LevSceneObjectTreeView::dropEvent(QDropEvent * event)
{
	QStandardItem* item = new QStandardItem;
	item->setText(QString(event->mimeData()->text()));
	m_next_update_items.push_back(item);
}
